...
document.addEventListener('DOMContentLoaded', function() {
    ...
    langSelects.forEach(function(el) {
        el.addEventListener('click', function(e) {
            ...
            // The most reliable way to trigger Google Translate is to set its cookie and reload
            function setCookie(key, value, expiry) {
              var expires = new Date();
              expires.setTime(expires.getTime() + (expiry * 24 * 60 * 60 * 1000));
              document.cookie = key + '=' + value + ';expires=' + expires.toUTCString() + ';path=/';
              // Also set it for the domain
              document.cookie = key + '=' + value + ';expires=' + expires.toUTCString() + ';domain=' + window.location.hostname + ';path=/';
            }
            
            // Setting cookie format: /pageLanguage/targetLanguage
            setCookie('googtrans', '/en/' + lang, 1);
            
            // Reload to apply the translation to the whole website instantly
